...
from effiswint import EffiSwinT
...
BASE_DIR = MODELS_DIR = DATA_DIR = TEST_DIR = OUTPUT_DIR = METADATA_PATH = TEST_LABELS_PATH = ''
...
def save_roc_curves(correct_labels, preds, model_name, accuracy, loss, f1):
  ...
  plt.title('ROC curve')
  plt.legend(loc='best')
  plt.savefig(os.path.join(OUTPUT_DIR, model_name +  "_" + opt.dataset + "_acc" + str(accuracy*100) + "_loss"+str(loss)+"_f1"+str(f1)+"_roc.jpg"))
  plt.clf()
  
def save_precision_recall_curve(correct_labels, preds, model_name, accuracy, loss, f1):
    plt.figure(1)
    plt.plot([0, 1], [0, 1], 'k--')
    precision, recall, threshold = metrics.precision_recall_curve(correct_labels, preds) 
    plt.plot(precision, recall, label="Model_"+ model_name)
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision Recall curve')
    plt.legend(loc='best')
    plt.savefig(os.path.join(OUTPUT_DIR, model_name +  "_" + opt.dataset + "_acc" + str(accuracy*100) + "_loss"+str(loss)+"_f1"+str(f1)+"_prcurve.jpg"))
    #print((os.path.join(OUTPUT_DIR, model_name +  "_" + opt.dataset + "_acc" + str(accuracy*100) + "_loss"+str(loss)+"_f1"+str(f1)+".jpg")))
    plt.clf()


def calculate_metrics(correct_labels, preds):
      fpr, tpr, th = metrics.roc_curve(correct_labels, preds)
      false_accept_rate = fpr
      false_reject_rate = 1 - tpr
      half_total_error_rate = (false_accept_rate + false_reject_rate)/2

      false_accept_rate = sum(false_accept_rate)/len(false_accept_rate)
      false_reject_rate = sum(false_reject_rate)/len(false_reject_rate)
      half_total_error_rate = sum(half_total_error_rate)/len(half_total_error_rate)

      
      return false_accept_rate, false_reject_rate, half_total_error_rate
...
if __name__ == "__main__":
    ...
    parser.add_argument('--models_dir', type=str, required = True,
                        help="Location to save checkpoint to.")
    ...
    far, frr, hter = calculate_metrics(correct_test_labels, preds)
    print(model_name, "False Accept Rate:", far, "False Reject Rate:", frr, "Half Total Error Rate:", hter)
    save_precision_recall_curve(correct_test_labels, preds, model_name, accuracy, loss, f1)