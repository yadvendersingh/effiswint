...
from manipulate import checkpoint_seq, named_apply

# from .vision_transformer import get_init_weights_vit

__all__ = ['EffiSwinT']  # model_registry will add each entrypoint fn to this
...
class EffiSwinT(nn.Module):
    """ Efficient Swin Transformer
    EfficientNet-B0 implementation of efficientnet_pytorch
    
    Swin transformer Implementation from -
    A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
          https://github.com/huggingface/pytorch-image-models/blob/main/timm/models/
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = EfficientNet.from_pretrained('efficientnet-b0')
#         self.patch_embed._fc = nn.Linear(self.patch_embed._fc.in_features, 1)
#         self.patch_embed.load_state_dict(torch.load("/projects/academic/courses/cse666/yadvende/Models/EfficientNet/EfficientNet_50Epoch/Efficient0_checkpoint0_All"))
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed.extract_features(x)
        x=x.reshape(x.shape[0],56,56,20)
        #print("After",x.shape)
        x = self.layers(x)
        x = self.norm(x)
        return x
