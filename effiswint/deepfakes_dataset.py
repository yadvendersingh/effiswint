...
from albumentations import Compose, RandomBrightnessContrast, \
    ...
    ShiftScaleRotate, ImageCompression, PadIfNeeded, GaussNoise, GaussianBlur, Rotate, CoarseDropout

from transforms.albu import IsotropicResize
from transforms.custom_albu import RemoveFacialFeatures
...
class DeepFakesDataset(Dataset):
    ...
    def create_train_transforms(self, size):
        return Compose([
            OneOf([
                RemoveFacialFeatures(p=1.0, choice = 0),
                RemoveFacialFeatures(p=1.0, choice = 1),
                RemoveFacialFeatures(p=1.0, choice = 2),
                RemoveFacialFeatures(p=1.0, choice = 3)
            ], p=1),
